...
interface StatCardProps {
  ...
  className?: string
  gradient?: 'blue' | 'green' | 'purple' | 'teal' | 'amber'
}
...
export function StatCard({ title, value, description, icon: Icon, trend, className, gradient = 'blue' }: StatCardProps) {
  return (
    <Card className={cn('relative overflow-hidden hover:!translate-y-0 group admin-stat-card', className)}>
      <CardContent className="p-0">
        ...
        <div className="relative p-6">
          <div className="flex items-start justify-between">
            ...
            <div className={cn('icon-box admin-stat-card__icon h-12 w-12 group-hover:scale-110 transition-transform duration-300')}>
