...
export function mapSubscription(raw: unknown): Subscription {
  ...
  return {
    ...
    product_slug: asString(product.slug) || undefined,
...
export function mapAdminPlan(raw: unknown) {
  ...
  return {
    ...
    product_name: asString(product.name, 'Product'),
    product_slug: asString(product.slug),
    ...
    max_students: asNumber(limits.max_students, 500),
      max_devices: asNumber(limits.max_devices, 1),
    max_customers: asNumber(limits.max_customers, 0),
    max_gst_invoices: asNumber(limits.max_gst_invoices, 0),
    max_non_gst_invoices: asNumber(limits.max_non_gst_invoices, 0),
    invoice_limit_period: asString(limits.invoice_limit_period, asString(item.billing_cycle) === 'yearly' ? 'yearly' : 'monthly'),
    limits: {
      max_users: asNumber(limits.max_users ?? limits.max_staff, 10),
...
export function mapAdminLicense(raw: unknown) {
  ...
  return {
    ...
    used_users: asNumber(asRecord(item.seat_usage).users),
    used_students: asNumber(asRecord(item.seat_usage).students),
    remaining_users: asNumber(asRecord(item.seat_usage).remaining_users, asNumber(asRecord(item.effective_limits).max_users, 10)),
    remaining_students: asNumber(asRecord(item.seat_usage).remaining_students, asNumber(asRecord(item.effective_limits).max_students, 500)),
    seat_usage_reported: asBool(asRecord(item.seat_usage).is_reported),
